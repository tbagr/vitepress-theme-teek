...
2. 然后参考 Teek 现有的任一 [语言文件](https://github.com/tbagr/vitepress-theme-teek/tree/master/vitepress-theme-teek/src/locale/lang)，将里面的内容拷贝到 `zh-tw.ts` 文件中，并将所有内容（Value）修改为 `zh-tw` 语言
...
export default defineConfig({
  ...
  themeConfig: {
    logo: "https://testingcf.jsdelivr.net/gh/tbagr/static/user/avatar1.png",
    search: {
      provider: "local",
    },
  },
...
const teekConfig = defineTeekConfig({
  blogger: {
    name: "FuRiNaHiD​​a​​​​",
    slogan: "I hope you enjoy the role that has been yours for the past five hundred years!",
  },
});
